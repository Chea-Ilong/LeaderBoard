    ...
    <div className="hidden lg:flex lg:items-center lg:gap-4 xl:gap-6 mb-8">
      {headerItems.slice(0, 3).map((item) => (
        <div key={item.label} className={`flex-shrink-0 ${item.width}`}>
          <div
            className="rounded-2xl px-4 py-4 text-center h-16 flex items-center justify-center text-white font-medium text-lg xl:text-xl"
      ...
      <div className="flex-1 min-w-0">
        <div className="bg-white rounded-2xl px-2 py-1 h-16 flex flex-col justify-center overflow-hidden">
          <div className="font-medium text-lg xl:text-xl text-center mb-1" style={{ color: COLORS.SECONDARY }}>
            Question
          </div>
          <div className="rounded-lg overflow-hidden" style={{ backgroundColor: COLORS.PRIMARY }}>
            <div className="grid grid-cols-6 h-6">
              {questions.map((num) => (
                <div
                  key={num}
                  className="flex items-center justify-center text-white font-medium text-sm xl:text-base border-r border-white/20 last:border-r-0"
                >
                  {num}
                </div>
              ))}
      ...
      <div className="flex-shrink-0 w-28 xl:w-32">
        <div
          className="rounded-2xl px-4 py-4 text-center h-16 flex items-center justify-center text-white font-medium text-lg xl:text-xl leading-tight"
          style={{ backgroundColor: COLORS.PRIMARY }}
        >
          Total Points
        </div>
